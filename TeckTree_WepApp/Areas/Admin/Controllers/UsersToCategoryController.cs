...
namespace TeckTree_WepApp.Areas.Admin.Controllers
{
    ...
    public class UsersToCategoryController : Controller
    {
        ...
        {
            var userSelectedForCategoryToAdd = await GetUsersForCategoryToAdd(usersCategoryListModel);
            var usersSelectedForCategoryToDelete = await GetUsersForCategoryToDelete(usersCategoryListModel.CategoryId);
            await _dataFunctions.UpdateUserCategoryEntityAsync( usersSelectedForCategoryToDelete , userSelectedForCategoryToAdd);
